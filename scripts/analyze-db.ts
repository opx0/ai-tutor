...
async function main() {
  ...
  courses.forEach((c) => {
    console.log(
      `  ${c.id} | "${c.title}" | user=${c.userId} | public=${c.isPublic} | modules=${c._count.modules}`,
    );
  });
  ...
  if (algoCourse) {
    const modules = await prisma.module.findMany({
      ...
      include: {
        lessons: {
          select: { id: true, title: true, visualization: true, content: true },
          orderBy: { order: "asc" },
        },
      },
      orderBy: { order: "asc" },
    ...
    modules.forEach((m) => {
      ...
      m.lessons.forEach((l) => {
        ...
        console.log(
          `    ${l.title} | content=${hasContent ? l.content!.length + "chars" : "EMPTY"} | viz=${vizInfo}`,
        );
      });
    });
  }
  ...
  const orphanModules = await prisma.module
    .findMany({
  ...
  const rawCheck =
    await prisma.$queryRaw`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'Lesson' ORDER BY ordinal_position;`;
