...
function htmlToMarkdown(html: string): string {
  ...
  md = md.replace(/<pre><code(?:\s+class="([^"]*)")?>([\s\S]*?)<\/code><\/pre>/g, (_, lang, code) => {
    // Detect language from class or content
    let language = ''
    if (lang) {
      language = lang.replace('language-', '')
    } else if (code.includes('def ') || code.includes('import ') || code.includes('for ') || code.includes('class ')) {
      language = 'python'
  ...
  md = md.replace(/<ol>([\s\S]*?)<\/ol>/g, (_, content) => {
    const items = [...content.matchAll(/<li>([\s\S]*?)<\/li>/g)].map((m, i) => `${i + 1}. ${m[1].trim()}`)
    return '\n' + items.join('\n') + '\n'
  })
...
async function main() {
  ...
  const courseYaml = yaml.stringify('', {
    title: 'DSA Mastery',
    description: 'Master Data Structures and Algorithms from memory models to dynamic programming. 14 phases, 51 lessons with interactive visualizations and LeetCode practice.',
    slug: 'dsa-mastery',
  ...
  const totalViz = allPhases.reduce((s, p) => s + p.lessons.filter(l => l.visualization).length, 0)
  console.log(`\n═══════════════════════════════════════`)
