...
const binaryTreeDfsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Binary tree with root 10. DFS (Depth-First Search) explores as deep as possible before backtracking. Three orders: inorder, preorder, postorder.",
      elements: [
        {
          type: "graph",
          id: "tree",
    ...
    {
      message:
        "Start at root 10. Go LEFT first. Recurse to 5, then go left again to 2. Node 2 has no children — it's a leaf. Visit 2.",
      elements: [
        {
          type: "graph",
          id: "tree",
    ...
    {
      message:
        "Back to 5. Left done, visit 5, then go RIGHT to 7. Visit 7 (leaf). Left subtree of root done: [2, 5, 7].",
      elements: [
        {
          type: "graph",
          id: "tree",
    ...
    {
      message:
        "Visit root 10. Now go RIGHT to 15. Same pattern: left(12) → visit 15 → right(20). Final: [2, 5, 7, 10, 12, 15, 20] — sorted!",
      elements: [
        {
          type: "graph",
          id: "tree",
          label: "Binary Tree — Inorder Complete!",
          layout: "tree",
          nodes: [
...
const bfsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "BFS (Breadth-First Search) explores the tree level by level using a QUEUE. Start by enqueuing the root.",
      elements: [
        {
          type: "graph",
          id: "tree",
    ...
    {
      message:
        "Dequeue 1 (level 0). Enqueue its children: 2, 3. Queue now holds the entire next level.",
      elements: [
        {
          type: "graph",
          id: "tree",
    ...
    {
      message:
        "Dequeue 4, 5, 6, 7. All are leaves — no children to enqueue. Queue empty. BFS complete: [1, 2, 3, 4, 5, 6, 7].",
      elements: [
        {
          type: "graph",
          id: "tree",
...
const trieViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        'A Trie (prefix tree) stores strings character by character. Each edge is a character. Shared prefixes share nodes. Let\'s insert "app", "apple", "apt".',
      elements: [
        {
          type: "graph",
          id: "trie",
    ...
    {
      message:
        'Insert "app": Create nodes a → p → p. Mark last p as end-of-word (*). Three characters, three edges.',
      elements: [
        {
          type: "graph",
          id: "trie",
          label: 'Trie — after "app"',
          layout: "tree",
          nodes: [
    ...
    {
      message:
        'Insert "apple": Follow existing path a→p→p, then ADD l→e. "app" prefix is shared — no duplication! Mark e as end-of-word.',
      elements: [
        {
          type: "graph",
          id: "trie",
          label: 'Trie — after "apple"',
          layout: "tree",
          nodes: [
            { id: "root", value: "ROOT", state: "default" },
            { id: "a", value: "a", state: "done" },
    ...
    {
      message:
        'Insert "apt": Follow a→p (shared), then branch at p with new edge "t". "ap" is the shared prefix. This branching is the Trie\'s power.',
      elements: [
        {
          type: "graph",
          id: "trie",
          label: "Trie — all 3 words",
          layout: "tree",
          nodes: [
            { id: "root", value: "ROOT", state: "default" },
            { id: "a", value: "a", state: "done" },
...
const subsetsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Generate all subsets of [1, 2, 3]. At each element, we CHOOSE to include it or SKIP it. This builds a binary decision tree.",
      elements: [
        {
          type: "graph",
          id: "dt",
          label: "Decision Tree",
          layout: "tree",
          nodes: [{ id: "root", value: "[]", state: "active" }],
          edges: [],
        },
        ...
        },
        {
          ...
          lines: [
            {
              text: "Backtracking: explore all choices, undo (backtrack) to try alternatives",
              kind: "info",
            },
    ...
    {
      message:
        "Include 1. Now decide on 2: include → [1,2], or skip → [1]. If include 2, decide on 3: include → [1,2,3] or skip → [1,2].",
      elements: [
        {
          type: "graph",
          id: "dt",
          label: "Decision Tree — left branch",
          layout: "tree",
          nodes: [
        ...
        },
        {
          ...
          label: "Subsets Found",
          items: [
            { value: "[1,2,3]", state: "done" },
            { value: "[1,2]", state: "done" },
          ],
        },
        {
          type: "log",
    ...
    {
      message:
        "Continue exploring: [1,3], [1], [2,3], [2], [3], []. Each leaf of the decision tree is a valid subset. 2^n = 8 subsets total.",
      elements: [
        {
          type: "graph",
          id: "dt",
          label: "Complete Decision Tree",
          layout: "tree",
          nodes: [
...
const minHeapViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Min-Heap: parent ≤ children. Stored as an array where parent(i) = floor((i-1)/2), left(i) = 2i+1, right(i) = 2i+2. Push 3, then 1.",
      elements: [
        {
          type: "graph",
          id: "heap",
          label: "Min-Heap (tree view)",
          layout: "tree",
          nodes: [
    ...
    {
      message:
        "Push 3: Add to end of array [5,8,10,3]. Node 3 at index 3. Parent = index 1 (value 8). 3 < 8 → swap! Sift up.",
      elements: [
        {
          type: "graph",
          id: "heap",
    ...
    {
      message:
        "3 < 5 → swap again! 3 becomes the root. Heap: [3, 5, 10, 8]. Push done in O(log n) — at most height swaps.",
      elements: [
        {
          type: "graph",
          id: "heap",
    ...
    {
      message:
        "Pop min: Remove root (3). Move last element (8) to root. Sift DOWN: compare 8 with children 5 and 10. 5 < 8 → swap. Heap restored: [5, 8, 10].",
      elements: [
        {
          type: "graph",
          id: "heap",
          label: "After pop() — sifted down",
          layout: "tree",
          nodes: [
...
const hashMapViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        'Hash Map: array of "buckets". hash(key) % size = bucket index. Insert ("alice", 90), ("bob", 85), ("eve", 78). Table size = 4.',
      elements: [
        {
          type: "grid",
          id: "ht",
          label: "Hash Table (4 buckets)",
          cells: [
    ...
    {
      message:
        'hash("alice") % 4 = 1. Place (alice, 90) in bucket 1. hash("bob") % 4 = 3. Place (bob, 85) in bucket 3. No collisions so far.',
      elements: [
        {
          type: "grid",
          id: "ht",
    ...
    {
      message:
        'COLLISION! hash("eve") % 4 = 1, same as alice. Solution: chaining — bucket 1 becomes a linked list: alice → eve. Lookup is still O(1) average.',
      elements: [
        {
          type: "grid",
          id: "ht",
          label: "Hash Table (with chaining)",
          cells: [
            [{ value: "empty", state: "default" }],
            [
              { value: "alice:90", state: "done" },
              { value: "→ eve:78", state: "highlight" },
            ],
            [{ value: "empty", state: "default" }],
            [{ value: "bob:85", state: "done" }],
          ],
...
const unionFindViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Union-Find: track connected components. Initially 5 separate nodes, each is its own root (parent[i] = i). rank[i] = 0.",
      elements: [
        {
          type: "graph",
          id: "uf",
    ...
    {
      message:
        "Union(0, 1): find(0)=0, find(1)=1. Different roots → merge! Attach 1 under 0 (by rank). parent[1] = 0. Components: 4.",
      elements: [
        {
          type: "graph",
          id: "uf",
          label: "After Union(0,1)",
          nodes: [
          ...
          edges: [{ source: "1", target: "0", state: "done", directed: true }],
        },
        {
          type: "array",
          id: "parent",
    ...
    {
      message:
        "Union(2,3): attach 3 under 2. Union(0,2): find(0)=0, find(2)=2. Attach smaller tree under larger. Now {0,1,2,3} is one component.",
      elements: [
        {
          type: "graph",
          id: "uf",
          label: "After Union(0,2)",
          nodes: [
...
const islandsBfsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Number of Islands: Count connected groups of 1s. Grid: 1=land, 0=water. BFS/DFS from each unvisited land cell and mark entire island as visited.",
      elements: [
        {
          type: "grid",
          id: "grid",
        ...
        { type: "variable", id: "v-islands", name: "islands", value: "0", state: "default" },
        {
          type: "log",
          id: "log",
          lines: [{ text: "Scan grid. Found land at (0,0) — start BFS!", kind: "call" }],
        },
      ],
    },
    {
      message:
        "BFS from (0,0): Visit (0,0), queue neighbors. (0,1)=land → visit. (1,0)=land → visit. Both have no more unvisited land neighbors. Island 1 complete!",
      elements: [
        {
          type: "grid",
          id: "grid",
    ...
    {
      message:
        "Found (1,3)=land. BFS: only (1,3) — surrounded by water on 3 sides and (2,3) below. Visit (2,3), then (2,2). Island 2 done. Continue scan — no more land. Total: 3 islands.",
      elements: [
        {
          type: "grid",
          id: "grid",
          label: "All Islands Found!",
          cells: [
            [
              { value: "1", state: "done" },
              { value: "1", state: "done" },
...
const dijkstraViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Dijkstra's: Find shortest path from A to all nodes. Initialize dist[A]=0, all others=∞. Use a min-heap to always process the closest unvisited node.",
      elements: [
        {
          type: "graph",
          id: "g",
    ...
    {
      message:
        "Process A: Relax A→B (0+4=4 < ∞), A→C (0+2=2 < ∞). Update dist[B]=4, dist[C]=2. Next: process C (smallest unvisited dist).",
      elements: [
        {
          type: "graph",
          id: "g",
    ...
    {
      message:
        "Process C: Relax C→B (2+1=3 < 4!) dist[B] updated to 3. C→E (2+5=7). Next: process B (dist=3).",
      elements: [
        {
          type: "graph",
          id: "g",
    ...
    {
      message:
        "Process B: B→D (3+3=6). Process D: D→E (6+1=7, not better than 7). Process E: no updates. Done! Shortest: A→C→B→D→E = 7.",
      elements: [
        {
          type: "graph",
          id: "g",
...
const topoSortViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Topological Sort: Order vertices of a DAG so every edge u→v has u before v. Use Kahn's BFS: start with nodes that have indegree 0.",
      elements: [
        {
          type: "graph",
          id: "dag",
          label: "Course Prerequisites (DAG)",
          nodes: [
    ...
    {
      message:
        "Process Math: add to order, decrement indegree of CS201 (2→1) and CS301 (2→1). Process CS101: decrement CS201 (1→0). CS201 indegree=0 → enqueue!",
      elements: [
        {
          type: "graph",
          id: "dag",
        ...
        },
        {
          ...
          label: "Topo Order",
          items: [
            { value: "Math", state: "done" },
            { value: "CS101", state: "done" },
          ],
        },
        {
          type: "log",
    ...
    {
      message:
        "Process CS201: CS301 (1→0), CS401 (1→0). Both enqueue. Process CS301, CS401. Done! Order: Math → CS101 → CS201 → CS301 → CS401.",
      elements: [
        {
          type: "graph",
          id: "dag",
          label: "Topological Order Found!",
          nodes: [
            { id: "A", value: "Math", state: "done", x: 0, y: 0 },
            { id: "B", value: "CS101", state: "done", x: 0, y: 100 },
        ...
        },
        {
          type: "array",
          id: "order",
          label: "Topo Order (complete!)",
          items: [
            { value: "Math", state: "done" },
            { value: "CS101", state: "done" },
            { value: "CS201", state: "done" },
            { value: "CS301", state: "done" },
            { value: "CS401", state: "done" },
          ],
        },
        {
          type: "log",
...
const kadanesViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Kadane's Algorithm: Find max subarray sum in [-2, 1, -3, 4, -1, 2, 1, -5, 4]. Track maxEndingHere and maxSoFar.",
      elements: [
        {
          type: "array",
          id: "arr",
        ...
        { type: "variable", id: "v-msf", name: "maxSoFar", value: "-2", state: "active" },
        {
          ...
          lines: [
            {
              text: "Key insight: at each position, either EXTEND the current subarray or START FRESH",
              kind: "info",
            },
    ...
    {
      message:
        "i=1: max(1, -2+1=-1) = 1. Starting fresh at 1 is better than extending! maxSoFar = 1. i=2: max(-3, 1-3=-2) = -2. Extend. maxSoFar stays 1.",
      elements: [
        {
          type: "array",
          id: "arr",
    ...
    {
      message:
        "i=3: max(4, -2+4=2) = 4. Start fresh! i=4: max(-1, 4-1=3) = 3. Extend. i=5: max(2, 3+2=5) = 5. Extend! maxSoFar = 5.",
      elements: [
        {
          type: "array",
          id: "arr",
    ...
    {
      message:
        "i=6: 5+1=6. maxSoFar=6! i=7: 6-5=1 > -5, extend. i=8: 1+4=5 > 4, extend. Done! Max subarray sum = 6 from [4,-1,2,1]. O(n) single pass.",
      elements: [
        {
          type: "array",
          id: "arr",
...
const climbingStairsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Climbing Stairs: n=5 steps. You can take 1 or 2 steps at a time. How many distinct ways to reach the top? dp[i] = ways to reach step i.",
      elements: [
        {
          type: "array",
          id: "dp",
        ...
        { type: "variable", id: "v-i", name: "computing", value: "dp[2]", state: "active" },
        {
          ...
          lines: [
            ...
            {
              text: "Why? To reach step i, either come from i-1 (+1 step) or i-2 (+2 steps)",
              kind: "info",
            },
          ],
        },
    ...
    {
      message:
        "dp[2] = dp[1] + dp[0] = 1 + 1 = 2. Two ways: (1+1) or (2). dp[3] = dp[2] + dp[1] = 2 + 1 = 3. Three ways: (1+1+1), (1+2), (2+1).",
      elements: [
        {
          type: "array",
          id: "dp",
    ...
    {
      message:
        "dp[4] = 3+2 = 5. dp[5] = 5+3 = 8. Answer: 8 ways to climb 5 stairs! O(n) time, O(n) space (or O(1) with just two variables).",
      elements: [
        {
          type: "array",
          id: "dp",
...
const uniquePathsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "2D DP: Unique Paths in a 3x4 grid. Can only move right or down. dp[r][c] = ways to reach cell (r,c). First row and first column are all 1 (only one direction).",
      elements: [
        {
          type: "grid",
          id: "dp",
          label: "dp[][] — Unique Paths",
          cells: [
            [
              { value: "1", state: "done" },
              { value: "1", state: "done" },
    ...
    {
      message:
        "Fill row by row. dp[1][1]=2, dp[1][2]=3, dp[1][3]=4. Each cell = sum of cell above + cell to the left.",
      elements: [
        {
          type: "grid",
          id: "dp",
    ...
    {
      message:
        "Fill row 2: dp[2][1]=1+2=3, dp[2][2]=3+3=6, dp[2][3]=4+6=10. Answer: 10 unique paths to bottom-right!",
      elements: [
        {
          type: "grid",
          id: "dp",
...
const knapsackViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "0/1 Knapsack: Items [(weight=1, val=6), (w=2, v=10), (w=3, v=12)], capacity=5. dp[i][w] = max value using first i items with capacity w.",
      elements: [
        {
          type: "grid",
          id: "dp",
          label: "dp[items][capacity]",
          cells: [
            [
              { value: "0", state: "done" },
              { value: "0", state: "done" },
    ...
    {
      message:
        "Row 1 (item1: w=1, v=6): For cap≥1, we CAN take item1. dp[1][w] = max(skip=0, take=6). All caps ≥ 1 get value 6.",
      elements: [
        {
          type: "grid",
          id: "dp",
    ...
    {
      message:
        "Row 2 (item2: w=2, v=10): cap=1: can't fit, skip → 6. cap=2: max(skip=6, take=0+10) = 10. cap=3: max(6, 6+10=16) = 16! Both items fit.",
      elements: [
        {
          type: "grid",
          id: "dp",
          ...
          cells: [
            ...
            [
              ...
              { value: "16", state: "highlight" },
              { value: "16", state: "active" },
              { value: "16", state: "active" },
            ],
            [
              { value: "0", state: "done" },
    ...
    {
      message:
        "Row 3 (item3: w=3, v=12): dp[3][5] = max(skip=16, take=dp[2][2]+12=22) = 22. Take items 2+3 (w=2+3=5, v=10+12=22). Optimal!",
      elements: [
        {
          type: "grid",
          id: "dp",
...
const bitOpsViz: VisualizationBlock = {
  ...
  steps: [
    {
      message:
        "Bit Manipulation: computers store everything as binary. Let's see AND, OR, XOR on 5 (0101) and 3 (0011).",
      elements: [
        {
          type: "array",
          id: "a",
        ...
        { type: "variable", id: "v-b", name: "b", value: "3 (0011)", state: "active" },
        {
          ...
          lines: [
            ...
            {
              text: "AND: both bits 1 → 1. OR: either bit 1 → 1. XOR: different → 1",
              kind: "info",
            },
          ],
        },
    ...
    {
      message:
        "AND (5 & 3): 0101 & 0011 = 0001 = 1. Only position 0 has both bits set. AND is useful for masking — extracting specific bits.",
      elements: [
        {
          type: "array",
          id: "a",
    ...
    {
      message:
        "XOR (5 ^ 3): 0101 ^ 0011 = 0110 = 6. XOR is 1 where bits DIFFER. Key property: a ^ a = 0, a ^ 0 = a. This finds the single unique number!",
      elements: [
        {
          type: "array",
          id: "a",
        ...
        { type: "variable", id: "v-res", name: "5 ^ 3", value: "6", state: "done" },
        {
          ...
          lines: [
            ...
            {
              text: "Magic: XOR all elements in [2,3,2] → 2^3^2 = 3. Duplicates cancel!",
              kind: "info",
            },
...
export const advancedPhases: DemoPhase[] = [
    ...
    lessons: [
      {
        ...
        content:
          "<h2>Core Idea</h2><p>A <strong>binary tree</strong> is a node with at most two children (left, right). A <strong>Binary Search Tree (BST)</strong> adds the invariant: left < node < right.</p><h2>DFS — Three Orders</h2><ul><li><strong>Inorder</strong> (left → node → right) — gives sorted output for BSTs</li><li><strong>Preorder</strong> (node → left → right) — useful for serialization</li><li><strong>Postorder</strong> (left → right → node) — useful for deletion, evaluation</li></ul><pre><code>def inorder(node):\n  if not node: return\n  inorder(node.left)\n  print(node.val)   # visit\n  inorder(node.right)</code></pre><h2>Complexity</h2><p>O(n) time (visit every node), O(h) space (call stack = height). Balanced: h = log n. Skewed: h = n.</p>",
        visualization: binaryTreeDfsViz,
      },
      {
        id: "B-17",
        code: "B-17",
        ...
        content:
          '<h2>When to Use BFS vs DFS</h2><ul><li><strong>BFS</strong>: level-order, shortest path in unweighted graph, minimum depth</li><li><strong>DFS</strong>: path finding, backtracking, checking properties (balanced, symmetric)</li></ul><h2>The BFS Template</h2><pre><code>from collections import deque\ndef bfs(root):\n  queue = deque([root])\n  while queue:\n    node = queue.popleft()\n    # process node\n    if node.left: queue.append(node.left)\n    if node.right: queue.append(node.right)</code></pre><h2>Level-Order Variation</h2><p>Process entire level at once by iterating <code>for _ in range(len(queue))</code> within the loop. Useful for "zigzag", "right side view", "level averages".</p>',
        visualization: bfsViz,
      },
      {
        id: "B-18",
        code: "B-18",
      ...
      },
      {
        ...
        title: "Segment Tree",
        content:
          "<h2>Why This Matters</h2><p>Prefix sums handle static arrays. But what if the array <strong>changes</strong>? Segment Tree answers range queries AND supports updates in O(log n).</p><h2>Core Idea</h2><p>Build a balanced binary tree where each node stores the aggregate (sum/min/max) of a range. Leaves = individual elements. Internal nodes = merge of children.</p><h2>Operations</h2><ul><li><strong>Build</strong> — O(n)</li><li><strong>Query(l, r)</strong> — O(log n): recursively combine relevant segments</li><li><strong>Update(i, val)</strong> — O(log n): update leaf, propagate up</li></ul><h2>When to Use</h2><p>Range sum + point update, range min/max queries, competitive programming. Overkill for interviews unless specifically asked.</p>",
        visualization: null,
      },
    ],
    bossChallenge:
      "Implement DFS inorder, preorder, and postorder iteratively (without recursion, using an explicit stack). Then implement BFS level-order.",
    leetcode: [
      {
        id: 226,
      ...
      {
        id: 105,
        title: "Build Tree from Pre/Inorder",
    ...
    lessons: [
      {
        ...
        title: "Subsets",
        content:
          "<h2>The Template</h2><pre><code>def backtrack(start, current):\n  result.append(current.copy())  # every state is a valid subset\n  for i in range(start, len(nums)):\n    current.append(nums[i])     # CHOOSE\n    backtrack(i + 1, current)    # EXPLORE\n    current.pop()                # UN-CHOOSE</code></pre><h2>Why It Works</h2><p>At each element, we either include it or skip it. The recursion tree has 2^n leaves — one per subset. The <code>start</code> parameter prevents duplicate subsets.</p><h2>Key Insight</h2><p>Backtracking is DFS on an <strong>implicit decision tree</strong>. The tree isn't stored in memory — we build/destroy it as we recurse.</p>",
        visualization: subsetsViz,
      },
      {
        id: "A-7",
        code: "A-7",
        title: "Permutations",
        content:
          "<h2>Permutations vs Subsets</h2><p>Subsets: choose/skip each element. Permutations: choose the <strong>order</strong> of all elements. For [1,2,3], there are 3! = 6 permutations.</p><h2>Template</h2><pre><code>def backtrack(current):\n  if len(current) == len(nums):\n    result.append(current.copy())\n    return\n  for num in nums:\n    if num in current: continue   # skip used\n    current.append(num)           # choose\n    backtrack(current)             # explore\n    current.pop()                  # un-choose</code></pre><p><em>[Optimization]</em> Use a <code>used</code> boolean array instead of <code>in</code> for O(1) lookup.</p>",
        visualization: null,
      },
      {
        id: "A-8",
        ...
        title: "N-Queens",
        content:
          "<h2>The Classic</h2><p>Place N queens on an N×N chessboard so no two queens attack each other (same row, column, or diagonal).</p><h2>Approach</h2><p>Place one queen per row. For each row, try each column. Before placing, check column and both diagonals.</p><h2>Pruning</h2><pre><code>cols = set()     # occupied columns\ndiag1 = set()    # row - col (main diagonal)\ndiag2 = set()    # row + col (anti-diagonal)\n\ndef backtrack(row):\n  if row == n: result.append(board.copy())\n  for col in range(n):\n    if col in cols or (row-col) in diag1 or (row+col) in diag2:\n      continue  # PRUNE — skip invalid placement\n    # place queen, recurse, remove queen</code></pre><p>Pruning is what makes backtracking efficient — we don't explore clearly invalid branches.</p>",
        visualization: null,
      },
    ],
    bossChallenge:
      "Solve N-Queens for N=4 on paper, drawing the decision tree. Then code it without looking at the template.",
    leetcode: [
      {
        id: 78,
        title: "Subsets",
    ...
    lessons: [
      {
        ...
        content:
          "<h2>What is a Heap?</h2><p>A <strong>min-heap</strong> is a complete binary tree where parent ≤ children. The minimum is always at the root. Stored as an array for efficiency.</p><h2>Key Operations</h2><ul><li><strong>push(x)</strong> — O(log n): add to end, sift up</li><li><strong>pop()</strong> — O(log n): swap root with last, remove last, sift down</li><li><strong>peek()</strong> — O(1): return root</li></ul><h2>Array Representation</h2><pre><code>parent(i)     = (i - 1) // 2\nleft_child(i)  = 2 * i + 1\nright_child(i) = 2 * i + 2</code></pre><p>Complete binary tree → no gaps in array → excellent cache performance.</p>",
        visualization: minHeapViz,
      },
      {
        id: "B-21",
        code: "B-21",
        ...
        content:
          "<h2>Top K Pattern</h2><p>Find the K largest elements: maintain a <strong>min-heap of size K</strong>. Push each element; if heap size > K, pop the min. Final heap = K largest.</p><pre><code>import heapq\ndef topK(nums, k):\n  heap = []\n  for num in nums:\n    heapq.heappush(heap, num)\n    if len(heap) > k:\n      heapq.heappop(heap)  # remove smallest\n  return heap</code></pre><p>O(n log k) — much better than O(n log n) sorting when k << n.</p><h2>Two Heaps: Running Median</h2><p>Max-heap for lower half, min-heap for upper half. Median = top of max-heap (or average of both tops). Each insert: O(log n).</p>",
        visualization: null,
      },
    ],
    bossChallenge:
      'Implement a min-heap class from scratch with push, pop, and peek. Then solve "K Closest Points to Origin" (LC 973).',
    leetcode: [
      {
        id: 215,
  ...
  // ─── Phase 11: Union-Find ─────────────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        content:
          "<h2>The Pattern</h2><p>Union-Find shines when you need to:</p><ol><li>Dynamically add connections between elements</li><li>Query whether two elements are connected</li><li>Count the number of distinct groups</li></ol><h2>Classic Problems</h2><ul><li><strong>Number of Connected Components</strong> — union edges, count distinct roots</li><li><strong>Redundant Connection</strong> — union each edge; if find(u)==find(v) before union, that edge creates a cycle</li><li><strong>Accounts Merge</strong> — union accounts that share an email</li></ul><h2>Union-Find vs BFS/DFS</h2><p>BFS/DFS works on static graphs. Union-Find is better when edges are <strong>added incrementally</strong> (streaming/online problems).</p>",
        visualization: null,
      },
    ],
    bossChallenge:
      "Implement Union-Find from memory. Then solve Redundant Connection (LC 684) — find the edge that creates a cycle.",
    leetcode: [
      {
        id: 200,
  ...
  // ─── Phase 12: Graphs ─────────────────────────────────────────────
  {
    id: "phase-12",
    phase: 12,
    title: "Graphs, Dijkstra & Topo Sort",
    ...
    lessons: [
      ...
      },
      {
        ...
        content:
          "<h2>Shortest Path in Weighted Graphs</h2><p>BFS finds shortest path in unweighted graphs. For weighted (non-negative) edges, use <strong>Dijkstra's</strong>.</p><h2>Algorithm</h2><ol><li>Set dist[source] = 0, all others = ∞</li><li>Use a min-heap. Push (0, source)</li><li>Pop minimum. For each neighbor, if dist[current] + weight < dist[neighbor], update and push</li></ol><h2>Why It's Greedy</h2><p>The min-heap always processes the node with smallest known distance. Once a node is popped, its distance is finalized — we've found the shortest path to it.</p><h2>Complexity</h2><p>O((V + E) log V) with a binary heap. O(V² + E) with a simple array (better for dense graphs).</p>",
        visualization: dijkstraViz,
      },
      {
        id: "A-11",
        code: "A-11",
    ...
    lessons: [
      {
        ...
        content:
          "<h2>Maximum Subarray Sum</h2><p>Given an array, find the contiguous subarray with the largest sum. Kadane's solves this in O(n) with O(1) space.</p><h2>The DP Insight</h2><p>At each index i, we decide: <strong>extend</strong> the current subarray, or <strong>start fresh</strong>.</p><pre><code>maxEndingHere = max(arr[i], maxEndingHere + arr[i])\nmaxSoFar = max(maxSoFar, maxEndingHere)</code></pre><p>If the running sum becomes negative, starting fresh is better than carrying the burden.</p><h2>Why This is DP</h2><p>dp[i] = max subarray ending at i = max(arr[i], dp[i-1] + arr[i]). We just use one variable instead of an array — <strong>space optimization</strong>.</p>",
        visualization: kadanesViz,
      },
      {
        id: "A-13",
        code: "A-13",
      ...
      },
      {
        ...
        title: "0/1 Knapsack",
        content:
          "<h2>The Classic DP Problem</h2><p>Given items with weights and values, and a knapsack with capacity W, maximize the total value without exceeding W.</p><h2>Recurrence</h2><pre><code>dp[i][w] = max(\n  dp[i-1][w],              # skip item i\n  dp[i-1][w-wt[i]] + val[i]  # take item i\n)</code></pre><p>For each item: either skip it (keep previous best) or take it (use remaining capacity).</p><h2>Variants</h2><ul><li><strong>0/1 Knapsack</strong> — each item used at most once</li><li><strong>Unbounded Knapsack</strong> — unlimited copies: dp[i][w-wt[i]] instead of dp[i-1][w-wt[i]]</li><li><strong>Subset Sum</strong> — special case where value = weight, target sum</li><li><strong>Coin Change</strong> — unbounded knapsack with minimum count instead of max value</li></ul>",
        visualization: knapsackViz,
      },
    ],
    bossChallenge:
      "Solve Longest Common Subsequence (LC 1143) from scratch. Define state, write recurrence, build the table. If you can do this cold, DP has clicked.",
    leetcode: [
      {
        id: 53,
  ...
  // ─── Phase 14: Bit Manipulation ───────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        title: "XOR Tricks",
        content:
          "<h2>XOR Properties</h2><ul><li><code>a ^ a = 0</code> — anything XOR itself is 0</li><li><code>a ^ 0 = a</code> — anything XOR 0 is itself</li><li><strong>Commutative & Associative</strong> — order doesn't matter</li></ul><h2>Single Number</h2><p>Array where every element appears twice except one. XOR all elements — duplicates cancel out, leaving the unique one.</p><pre><code>def singleNumber(nums):\n  result = 0\n  for num in nums:\n    result ^= num\n  return result</code></pre><p>O(n) time, O(1) space — no hash set needed!</p><h2>Swap Without Temp Variable</h2><pre><code>a ^= b\nb ^= a\na ^= b</code></pre><p>Mind-bending but works because XOR is its own inverse.</p>",
        visualization: null,
      },
      {
        id: "B-28",
        ...
        title: "Counting Bits",
        content:
          "<h2>Count Set Bits (Hamming Weight)</h2><pre><code>def countBits(n):\n  count = 0\n  while n:\n    count += n & 1\n    n >>= 1\n  return count</code></pre><h2>Brian Kernighan's Trick</h2><pre><code>def countBits(n):\n  count = 0\n  while n:\n    n &= n - 1  # clears the lowest set bit!\n    count += 1\n  return count</code></pre><p><code>n & (n-1)</code> removes the rightmost 1-bit. So this loops exactly (number of 1-bits) times.</p><h2>DP Version — Count Bits 0 to n</h2><pre><code>dp[i] = dp[i >> 1] + (i & 1)</code></pre><p>Shift right removes last bit (already solved), add back that last bit. O(n) for all values.</p>",
        visualization: null,
      },
    ],
    bossChallenge:
      'Solve "Single Number" (LC 136) and "Number of 1 Bits" (LC 191) in under 2 minutes each. Then explain WHY n & (n-1) clears the lowest set bit.',
    leetcode: [
      {
        id: 136,
        title: "Single Number",
