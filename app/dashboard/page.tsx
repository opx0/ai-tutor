  ...
  return (
    <div className="flex justify-center items-center h-[50vh]">
      <div className="relative w-16 h-16">
        ...
        <div
          className="absolute inset-2 rounded-full border-r-2 border-primary animate-spin"
...
export default async function DashboardPage() {
  ...
  // Batch all progress rows in a single query instead of one per course (N+1).
  const progressRows = await prisma.userProgress.findMany({
    where: { userId: user.id, courseId: { in: courses.map((c) => c.id) } },
    select: { courseId: true, progress: true },
  });
  const progressByCourse = new Map(progressRows.map((p) => [p.courseId, p.progress]));
    ...
    include: {
      lesson: {
        select: {
          id: true,
          title: true,
          module: { select: { title: true, course: { select: { slug: true, id: true } } } },
        },
      },
    },
  });
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/courses?tab=my"
            className="bg-card hover:bg-accent border border-border/50 text-foreground px-4 py-2 rounded-full text-sm font-semibold transition-colors flex items-center gap-2"
          >
            <BookOpen className="w-4 h-4" /> Learning path
          </Link>
          <Link
            href="/courses"
            className="bg-chart-2 hover:bg-chart-2/90 text-background px-5 py-2 rounded-full text-sm font-bold shadow-md transition-all hover:shadow-lg flex items-center gap-2"
          >
          ...
          <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-card border border-border/40 rounded-3xl p-6 flex flex-col justify-between relative overflow-hidden group shadow-sm">
              ...
              <div className="flex items-end gap-3 relative z-10">
                ...
                <span className="px-2 py-0.5 rounded-full bg-chart-2/20 text-chart-2 text-[10px] uppercase font-bold mb-1.5 tracking-wider w-max">
                  Average Status
                </span>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/40 rounded-3xl p-6 flex flex-col justify-between shadow-sm group relative overflow-hidden">
              ...
              <div className="flex items-end gap-3 relative z-10">
                ...
                <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] uppercase font-bold mb-1.5 tracking-wider w-max">
                  In Progress
                </span>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/40 rounded-3xl p-6 flex flex-col justify-between shadow-sm group relative overflow-hidden">
              ...
              <div className="flex items-end gap-3 relative z-10">
                ...
                <span className="px-2 py-0.5 rounded-full bg-chart-4/10 text-chart-4 text-[10px] uppercase font-bold mb-1.5 tracking-wider w-max">
                  Total Sessions
                </span>
              </div>
            </div>
                ...
                totalTimeString={
                  <>
                    {totalHrs > 0 && (
                      <>
                        {totalHrs}{" "}
                        <span className="text-xl font-medium text-muted-foreground tracking-normal">
                          h
                        </span>
                      </>
                    )}
                ...
                <div className="space-y-6 relative z-10">
                  {upcomingReviews.length > 0 ? (
                    upcomingReviews.map((review, idx) => {
                      ...
                      const courseIdParam =
                        review.lesson.module.course.slug || review.lesson.module.course.id;

                      return (
                        <div key={review.id} className="flex gap-4 items-center group">
                          <div
                            className={`w-12 text-right text-xs font-bold ${isToday ? "text-foreground" : "text-muted-foreground"}`}
                          >
                            {isToday
                              ? "Today"
                              : reviewTime.toLocaleDateString(undefined, {
                                  month: "short",
                                  day: "numeric",
                                })}
                          </div>
                          <div
                            className={`w-3 h-3 rounded-full border-2 border-background z-10 transition-colors ${isToday ? "bg-primary shadow-md" : "bg-muted-foreground"}`}
                          />

                          <Link
                            href={`/courses/${courseIdParam}/${review.lesson.id}`}
                            className="flex-1 bg-muted/20 border border-border/30 rounded-2xl p-4 flex justify-between items-center transition-colors hover:bg-muted/50 hover:border-border/60"
                          >
                            <div>
                            ...
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isToday ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                            >
                    ...
                    <div className="flex flex-col items-center justify-center py-6 text-center opacity-70">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Keep completing lessons to build your queue!
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="flex-1 flex flex-col gap-3">
              {coursesWithProgress.slice(0, 4).map((course, i) => {
                ...
                return (
                  <Link
                    ...
                    className="group relative bg-muted/20 border border-border/40 rounded-2xl p-4 hover:bg-muted/40 transition-all flex items-center gap-4"
                  >
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${colors[colorIdx]}`}
                    >
                      <BookOpen className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex-1 h-1.5 bg-background rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${glowColors[colorIdx]}`}
                      ...
                      <p className="text-[10px] text-muted-foreground mt-1 tracking-wide">
                        ~ {course._count.lessons * 20}m remain
                      </p>
                    </div>
              ...
              {coursesWithProgress.length === 0 && (
                <div className="flex flex-col items-center justify-center py-8 text-center h-full">
                  ...
                  <Link
                    href="/courses?tab=create"
                    className="text-xs font-bold text-chart-2 mt-2 hover:underline"
                  >
                    Create a Course
                  </Link>
                </div>
