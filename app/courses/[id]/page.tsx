...
export default async function CoursePage({ params }: PageProps) {
  ...
  if (isCurated) {
    ...
    const activeLesson =
      activeModule?.lessons.find((l) => !completedLessonIds.includes(l.id)) ||
      activeModule?.lessons[activeModule.lessons.length - 1];

    const activeModCompleted =
      activeModule?.lessons.filter((l) => completedLessonIds.includes(l.id)).length || 0;
        ...
        <div className="p-6 lg:p-8 overflow-auto relative flex-1">
          <div className="max-w-3xl mx-auto">
            ...
            {activeLesson && (
              <div className="mb-10">
                ...
                >
                  ...
                  <div className="absolute inset-0 z-0 p-[2px] rounded-2xl overflow-hidden pointer-events-none">
                    <div
                  ...
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, transparent 40%, ${accentColor})`,
                    }}
                  />
                    ...
                    <div className="flex-1">
                      ...
                      >
                        ...
                        <span className="text-muted-foreground">
                          Lesson {activeLesson.order + 1} of {activeModTotal}
                        </span>
                      </div>
            ...
            <div className="flex items-end justify-between mb-4">
              <h2 className="text-lg font-bold tracking-tight text-foreground">Course Outline</h2>
                  ...
                  >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: isComplete
                            ? `0 0 30px -5px ${accentColor}60, inset 0 0 0 1px ${accentColor}50`
                            : `inset 0 0 0 1px hsl(var(--border)/0.5)`,
                          textShadow: isComplete ? `0 0 10px ${accentColor}80` : "none",
                        }}
                      >
                        {isComplete ? <CircleCheck className="w-6 h-6" /> : mod.order + 1}
                      </div>
                    ...
                    <div className="flex-1 flex items-center justify-between sm:pl-6 sm:border-l border-white/10 dark:border-white/5 gap-5 relative z-10">
                      <div className="flex flex-col gap-2 flex-1">
                        <div
                          className={`flex items-center px-2 py-0.5 rounded text-[10px] font-bold tracking-wider w-max ${diffBadge.bg} ${diffBadge.color}`}
                        >
                          {diffBadge.label}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        <div className="hidden sm:block text-right w-24">
                          <div
                            className="flex items-center justify-end gap-1.5 text-xs font-semibold mb-1"
                            style={{ color: modPercent > 0 ? accentColor : undefined }}
                          >
                            {modPercent}%
                          </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <CourseDetail course={course} progress={progress} lastLessonId={lastLessonId} />
    </div>
  );
}
