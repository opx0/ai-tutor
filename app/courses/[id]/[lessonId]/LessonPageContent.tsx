...
import { ChevronLeft, ChevronRight, Edit3, GraduationCap, NotebookPen } from "lucide-react";
...
const ScenePlayer = dynamic(() => import("@/components/visualization/ScenePlayer"), { ssr: false });
      ...
      <div className="max-w-[720px] mx-auto px-4 sm:px-6 pb-24">
            ...
            <ReactMarkdown
              ...
              components={{
                pre: ({ node, children, ...props }) => {
                  ...
                  if (
                    child?.type === "code" ||
                    (child?.props?.className && typeof child?.props?.children === "string")
                  ) {
                  ...
                  return (
                    <pre
                      className="bg-black/90 p-4 rounded-xl overflow-auto my-6 border border-white/10 shadow-lg text-sm"
                      {...props}
                    >
                      {children}
                    </pre>
                ...
                ul: ({ node, ...props }) => (
                  <ul
                    className="list-disc pl-6 my-5 space-y-2 marker:text-muted-foreground"
                    {...props}
                  />
                ),
                ol: ({ node, ...props }) => (
                  <ol
                    className="list-decimal pl-6 my-5 space-y-2 marker:text-muted-foreground font-medium [&>li]:font-normal"
                    {...props}
                  />
                ),
                ...
                h1: ({ node, ...props }) => (
                  <h1
                    className="text-3xl font-bold mt-12 mb-6 tracking-tight text-foreground"
                    {...props}
                  />
                ),
                h2: ({ node, ...props }) => (
                  <h2
                    className="text-2xl font-bold mt-10 mb-5 tracking-tight text-foreground"
                    {...props}
                  />
                ),
                h3: ({ node, ...props }) => (
                  <h3
                    className="text-xl font-bold mt-8 mb-4 tracking-tight text-foreground"
                    {...props}
                  />
                ),
                p: ({ node, ...props }) => (
        ...
        {lesson.visualization && (
          <div className="mt-16 mb-8">
            <h2 className="text-2xl font-bold mb-6 tracking-tight">Interactive Visualization</h2>
        ...
        <div className="mt-16 pt-8 flex flex-col sm:flex-row gap-4 justify-between items-center sm:items-stretch">
          ...
          {nextLesson ? (
            ...
            >
              <div className="font-bold mb-0.5 flex items-center">Complete Course</div>
              <div className="text-xs text-white/80">Return to outline</div>
            </Link>
          )}
        </div>
      </div>
    ...
    return (
      <div className="flex-1 flex min-h-screen relative">
        ...
        <div className="flex-1 overflow-auto bg-background relative flex flex-col">
          ...
          <div
            className="h-14 flex items-center px-8 border-b border-border/5 shrink-0 sticky top-0 bg-background/80 backdrop-blur-xl z-10 w-full"
            style={{ borderBottomColor: `${accentColor}10` }}
          >
            <div
              className="flex items-center text-xs font-semibold tracking-wider hover:text-foreground transition-colors"
              style={{ color: accentColor }}
            >
          ...
          <div className="flex-1 p-6 lg:p-12 lg:max-w-4xl max-w-full mx-auto w-full">{content}</div>
        </div>
      </div>
    );
  }
