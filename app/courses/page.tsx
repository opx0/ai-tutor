...
import { Sparkles, BookOpen, LayoutGrid, Plus, ChevronRight, GraduationCap, CheckCircle2, Lock } from "lucide-react";
    ...
    <div className="min-h-screen relative overflow-hidden bg-background">
      ...
      <div className="container mx-auto px-4 sm:px-8 pt-28 pb-12 max-w-7xl relative z-10">
        ...
        <Tabs defaultValue={defaultTab} className="w-full">
          ...
          {/* Tab 1: Curated Paths */}
          <TabsContent value="curated" className="mt-0 outline-none animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            {data.courses.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center bg-card/40 backdrop-blur-sm border border-border/50 rounded-3xl mt-8 shadow-sm">
                ...
                <p className="text-muted-foreground text-sm max-w-sm">Curated courses are being forged by our curriculum team.</p>
              </div>
            ) : (
                      ...
                      <div className="relative p-8 sm:p-10 z-10 flex flex-col h-full justify-between gap-8">
                        <div>
                          <div className="flex items-center justify-between mb-6">
                            <div className="px-3 py-1 text-xs font-black uppercase tracking-wider rounded-lg backdrop-blur-md border" style={{ color: isLocked ? 'gray' : accentColor, backgroundColor: isLocked ? 'transparent' : `${accentColor}10`, borderColor: isLocked ? 'gray' : `${accentColor}30` }}>
                          ...
                          <p className="text-muted-foreground leading-relaxed sm:text-lg max-w-md">
                            {course.description || "Master the core concepts with this structured, rigorous path."}
                          </p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-6 sm:items-end justify-between mt-auto pt-6 border-t border-white/10">
                          <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                            <div>
                              <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Modules</div>
                              <div className="font-semibold text-foreground">{course.moduleCount}</div>
                            </div>
                            <div>
                              <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Lessons</div>
                              <div className="font-semibold text-foreground">{course.lessonCount}</div>
                            </div>
                            <div>
                              <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Estimate</div>
                              <div className="font-semibold text-foreground">~{course.estimatedHours || 0}h</div>
                            </div>
                            <div>
                              <div className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Progress</div>
                              <div className="font-semibold" style={{ color: isCompleted ? '#4ade80' : accentColor }}>
          ...
          {/* Tab 2: My Library */}
          <TabsContent value="my" className="mt-0 outline-none animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
          ...
          {/* Tab 3: Create Course */}
          <TabsContent value="create" className="mt-0 outline-none animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
            <div className="max-w-3xl mx-auto bg-black/20 p-6 sm:p-10 rounded-[32px] border border-white/5 backdrop-blur-xl">
              <div className="mb-8 text-center">
                ...
                <p className="text-muted-foreground text-sm">Design structured curriculum using AI generation or manual assembly.</p>
              </div>
              <CourseForm />
            </div>
          </TabsContent>
