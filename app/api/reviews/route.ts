...
export async function POST(request: Request) {
  ...
  const limit = await consumeRateLimit(`reviews:${session.user.id}:${clientId}`, {
    max: 60,
    windowMs: 60000,
  });
  if (!limit.allowed) {
  ...
  const parsed = reviewPostSchema.safeParse(await request.json());
  if (!parsed.success) {
    return NextResponse.json({ error: "lessonId is required" }, { status: 400 });
  }
  const { lessonId, quality, enqueue } = parsed.data;
  ...
  if (!existing) {
    return NextResponse.json({ error: "No review found for this lesson" }, { status: 404 });
  }
