...
import { ArrowLeft, Circle, CircleCheck, LayoutDashboard } from "lucide-react";
...
export default function CourseSidebar({
  ...
  const defaultOpenModule = (() => {
    if (activeLessonId) {
      const mod = modules.find((m) => m.lessons.some((l) => l.id === activeLessonId));
      if (mod) return mod;
    }
    return modules.find((m) => m.lessons.some((l) => !completedLessonIds.includes(l.id)));
  })();
    ...
    <aside className="w-[280px] border-r border-white/5 bg-[#0a0e14] shrink-0 hidden md:flex flex-col sticky top-0 h-screen z-20 shadow-[4px_0_24px_-10px_rgba(0,0,0,0.5)]">
      ...
      <ScrollArea className="flex-1 w-full">
        <div className="p-3 pb-24">
          <Accordion type="single" collapsible defaultValue={defaultOpenModule?.id}>
            {modules.map((mod) => {
              return (
                <AccordionItem
                  key={mod.id}
                          ...
                          >
                            <div className="pt-[2px] shrink-0 bg-transparent transition-colors">
                              {isCompleted ? (
                                ...
                                <Circle
                                  className={`h-4 w-4 transition-colors ${isActive ? "text-foreground drop-shadow-[0_0_5px_currentColor]" : "text-muted-foreground/30 group-hover:text-muted-foreground/60"}`}
                                />
                              )}
                            </div>

                            <div className="flex-1 min-w-0">
                              <div className="leading-snug truncate pr-1">{lesson.title}</div>
                            </div>
                          </Link>
                        );
                      })}
