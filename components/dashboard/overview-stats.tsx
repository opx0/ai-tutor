      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-chart-2 font-medium flex items-center gap-1 mt-1">
            <span className="w-2 h-2 rounded-full bg-chart-2 inline-block animate-pulse" /> Well time spent!
          </p>
        </div>
            ...
            >
              {data.map((entry, index) => (
                <Cell 
                  ...
                  fill={index === maxDayIndex && maxHours > 0 ? 'hsl(var(--chart-2))' : 'hsl(var(--primary)/0.2)'} 
                />
              ))}
            </Bar>
          </BarChart>
